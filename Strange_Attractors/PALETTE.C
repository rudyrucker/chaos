...
int LoadOurPalette(char *filename)
{
	...
	while (1)
	{
      ...
      if (tbuf[0] == '#')
		...
		if (sscanf(tbuf, "%d: %d %d %d", &ti, &tr, &tg, &tb) == 4)
		{
			...
			ourcolortable[ti - 1][0] = tr;
			ourcolortable[ti - 1][1] = tg;
			ourcolortable[ti - 1][2] = tb;
		}
	}
	...
	if (!vgaflag)
	{
		...
		for (i = 0; i < 16; i++)
			ourct[i] = EGA_equivalent(ourcolortable[i]);
	}
	else
	{
		for (i = 0; i < 16; i++)
...
void SaveOurPalette(char *filename)
{
	...
	fprintf(fd, "CHAOS Barnsley Color Table\n");
	fprintf(fd, "Created: %s", ctime(&timex));
	fprintf(fd, "Mode: %s\n", vgaflag ? "VGA" : "EGA");
	for (i = 0; i < 16; i++)
		fprintf(fd, "%02d: %02d %02d %02d\n", i + 1,
			ourcolortable[ourct[i]][0],
			ourcolortable[ourct[i]][1],
			ourcolortable[ourct[i]][2]);
...
{
	char *p = searchpath(filename);

	if (p && LoadOurPalette(p))
	{
		if (mode == 0x12)
		{
			int i;
...
{
	extern void _useEGApalette(unsigned char *);
	extern void _useVGApalette(unsigned char *);
...
void update_numbers(int current_color)
{
	...
	PenColor(BLACK);
	BackColor(LIGHTGRAY);
	sprintf(tbuf, "%2.2d", ourcolortable[ourct[current_color]][0]);
	DrawString(tbuf);
	MoveTo(Gslider.sR.Xmax + 4, Gslider.sR.Ymin - 2);
	sprintf(tbuf, "%2.2d", ourcolortable[ourct[current_color]][1]);
	DrawString(tbuf);
	MoveTo(Bslider.sR.Xmax + 4, Bslider.sR.Ymin - 2);
	sprintf(tbuf, "%2.2d", ourcolortable[ourct[current_color]][2]);
	DrawString(tbuf);
	ShowCursor();
}
...
{
	Rslider.value = ourcolortable[ourct[current_color]][0];
	Gslider.value = ourcolortable[ourct[current_color]][1];
	Bslider.value = ourcolortable[ourct[current_color]][2];
}
...
static void update_everything(slider * s, int current_color, int i)
{
	...
	ourcolortable[ourct[current_color]][i] = s->value;
	HideCursor();

	if (!vgaflag)
	{
		int ec = EGA_equivalent(ourcolortable[ourct[current_color]]);

		ourct[current_color] = ec;
		memcpy(ourcolortable[ourct[current_color]], defaultpalette[ec], 3);
		regs.h.al = 0;
		regs.h.ah = 0x10;
		regs.h.bh = ec;
	...
	else
	{
		...
		regs.h.ch = ourcolortable[ourct[current_color]][1];
		regs.h.cl = ourcolortable[ourct[current_color]][2];
		regs.h.dh = ourcolortable[ourct[current_color]][0];
...
//	ArrowCursor();
	...
	height = 2 * (width - 12) / 8 + 5 + 3 * (3 * FontHeight / 2)
	...
	for (i = 0; i < 16; i++)
	{
		...
		S.Ymin = tR.Ymin + FontHeight + 4 + (i / 8) * boxwidth;
		S.Ymax = S.Ymin + boxwidth - 2;
	...
	create_slider_base_width(&Rslider, &tR, 20, 2 * boxwidth + 5, false, false, false, false, bubblewidth);
	bR[items++] = &Rslider.bR;
	create_slider_base_width(&Gslider, &tR, 20, 2 * boxwidth + 5 + FontHeight + 4, false, false, false, false, bubblewidth);
	bR[items++] = &Gslider.bR;
	create_slider_base_width(&Bslider, &tR, 20, 2 * boxwidth + 5 + 2 * (FontHeight + 4), false, false, false, false, bubblewidth);
	bR[items++] = &Bslider.bR;
	PenColor(BLACK);
	BackColor(LIGHTGRAY);
...
//				ArrowCursor();
			...
			else if (current_item == items - 5)
			{
				if (select_file("Load Palette", "*.pal", tbuf, "PAL"))
				{
					...
					{
						/*
						 * we ned to rewrite a mess
						 * here
						 */
						/*
						 * Now tell the world of our
						 * palette
						 */
	...
	{
		memcpy(egacolortable, ourct, sizeof ourct);
		if (hasVGA)
			memcpy(vgacolortable, ourcolortable, sizeof ourcolortable);
	}
}
