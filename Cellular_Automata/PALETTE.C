...
void SaveOurPalette(char *filename)
{
	...
	fprintf(fd, "CHAOS Toy Universe Color Table\n");
...
{
   /* For this one, we search the path. */
   char *p = searchpath(filename);
	...
	memcpy(startvgapalette, ourcolortable, 16 * 3);
	memcpy(startegapalette, ourct, 16);
}
...
#pragma warn -ucp
void load_preset_palettes(void)
{
   int i;
   static struct {
      char *table;
      ...
      char *defct;
         ...
         HODGE_colortable,HODGE_ct,defHODGEpal,defHODGEct,
         NLUKY_colortable,NLUKY_ct,defNLUKYpal,defNLUKYct,
         EAT_colortable,EAT_ct,defEATpal,defEATct,
         TUBE_colortable,TUBE_ct,defTUBEpal,defTUBEct
      };

   for(i=0;i<16;i++)
      egacolortable[i] = i;

   for(i=0,pp=pt;i<4;i++,pp++)
   {
      char tbuf[12];
      sprintf(tbuf,"%s.pal",typenames[i]);
      if (LoadOurPalette(tbuf))
      {
         memcpy(pp->table,ourcolortable,16*3);
         if (!hasVGA)
            memcpy(pp->ct,ourct,16);
      }
      memcpy(pp->deft,pp->table,16*3);
      memcpy(pp->defct,(hasVGA) ? egacolortable : pp->ct,16);
   }

   memcpy(vgacolortable,HODGE_colortable,16*3);
   if (!hasVGA)
      memcpy(egacolortable,HODGE_ct,16);
