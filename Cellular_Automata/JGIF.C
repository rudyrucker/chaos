...
static unsigned char egadefaults[] = {
	0, 1, 2, 3, 4, 5, 0x14, 7,
...
{
	int i, j;
	unsigned char current_palette[16][3];
	unsigned char palette_order[17];
   ...
   HideCursor();
	if (hasVGA)
	{
      memcpy(current_palette,vgacolortable,16*3);
      memcpy(palette_order,egacolortable,16);
	}
	else
	{

		/* Load the current palette from the current palette number */

		char *ct = egacolortable;

		for (i = 0; i < 16; i++)
			for (j = 0; j < 3; j++)
				current_palette[i][j] = defaultpalette[ct[i]][j];
	}
	...
	gif.w = gim.w = sR.Xmax - (wholescreen ? 0 : 160) + 1;
	gif.h = gim.h = sR.Ymax + 1;
	...
	for (i = 0; i < 16; i++)
		for (j = 0; j < 3; j++)
			putc(conversion_table[current_palette[palette_order[i]][j]], giffd);
...
int gif_out_line(unsigned char far * pixels, int len)
{
   ...
   if (OurMode != 0x13)
   {
      if (gifrow + gim.y < (hasVGA ? 479 : 349))
      {
...
int GifDisplay(char *filename)
{
	...
	if (!memok(16133L)) {	       /* Added mallocs in decoder.c  */
